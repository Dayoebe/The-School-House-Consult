                ...
                @if($section)
                    @if($section === 'consultations')
                        <section class="rounded-[28px] border border-[#dce7e1] bg-white p-7 shadow-[0_12px_35px_rgba(11,42,91,.05)] sm:p-10">
                            <span class="inline-flex rounded-full bg-[#fff3ee] px-3 py-1 text-[10px] font-bold uppercase tracking-[.15em] text-coral">Inbox / consultation forms</span>
                            <h2 class="mt-5 font-display text-4xl font-semibold tracking-[-.05em] text-navy">Consultation requests</h2>
                            <p class="mt-3 text-sm text-muted">Every request submitted through the consultation form appears here.</p>
                            <div class="mt-8 space-y-4">@forelse($consultations as $consultation)<article class="rounded-2xl border border-[#e5ede8] bg-[#fbfdfb] p-5"><div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-start"><div><h3 class="font-display text-xl font-semibold text-navy">{{ $consultation->full_name }}</h3><p class="mt-1 text-sm text-muted">{{ $consultation->email }} · {{ $consultation->phone }}</p><p class="mt-1 text-sm text-muted">{{ $consultation->organisation ?: 'No organisation supplied' }} · {{ $consultation->organisation_type }}</p></div><form method="POST" action="{{ route('admin.consultations.status', $consultation) }}">@csrf @method('PATCH')<select class="rounded-lg border border-[#cbd8d2] bg-white px-3 py-2 text-xs font-semibold text-navy" name="status" onchange="this.form.submit()"><option value="new" @selected($consultation->status === 'new')>New</option><option value="in-progress" @selected($consultation->status === 'in-progress')>In progress</option><option value="resolved" @selected($consultation->status === 'resolved')>Resolved</option><option value="archived" @selected($consultation->status === 'archived')>Archived</option></select></form></div><div class="mt-4 grid gap-3 border-t border-[#e5ede8] pt-4 text-sm text-muted sm:grid-cols-2"><p><strong class="text-navy">Service:</strong> {{ $consultation->service?->title ?? 'General guidance' }}</p><p><strong class="text-navy">Preferred contact:</strong> {{ $consultation->preferred_contact_method }}</p></div><p class="mt-4 whitespace-pre-line rounded-xl bg-white p-4 text-sm leading-6 text-muted">{{ $consultation->message }}</p><p class="mt-3 text-xs text-muted">Received {{ $consultation->created_at->format('j M Y, g:i A') }}</p></article>@empty<p class="rounded-2xl bg-[#f0f7f3] p-8 text-sm text-muted">No consultation requests have been submitted yet.</p>@endforelse</div>
                        </section>
