...
<dialog id="mobile-menu" class="m-auto w-[min(480px,calc(100%-32px))] max-h-[calc(100dvh-32px)] rounded-[18px] bg-white p-7 text-ink backdrop:bg-[rgb(5_21_45_/_60%)] max-[767px]:mb-0 max-[767px]:w-full max-[767px]:max-w-none max-[767px]:rounded-t-[24px] max-[767px]:p-[25px_22px]" aria-labelledby="menu-title">
    ...
    <a class="mt-6 inline-flex w-full items-center justify-center gap-3 rounded border border-orange bg-orange px-6 py-[15px] font-bold text-[#14233a]" href="{{ route('contact') }}#consultation"><i class="fa-solid fa-calendar-check" aria-hidden="true"></i> Request a Consultation</a>
    <div class="flex flex-wrap justify-center gap-5 py-[15px] text-[13px] text-navy"><a class="flex items-center gap-1.5 px-1" href="tel:+2347062220159"><i class="fa-solid fa-phone text-[11px] text-teal" aria-hidden="true"></i> Call us</a><a class="flex items-center gap-1.5 px-1" href="{{ config('site.whatsapp') }}"><i class="fa-brands fa-whatsapp text-[15px] text-teal" aria-hidden="true"></i> WhatsApp</a>@auth @if(auth()->user()->isAdmin())<a class="flex items-center gap-1.5 px-1 font-semibold text-teal" href="{{ route('admin.dashboard') }}"><i class="fa-solid fa-gauge-high text-[11px]" aria-hidden="true"></i> Dashboard</a>@endif<form method="POST" action="{{ route('logout') }}" class="inline">@csrf<button class="flex items-center gap-1.5 px-1 font-semibold text-teal" type="submit"><i class="fa-solid fa-arrow-right-from-bracket text-[11px]" aria-hidden="true"></i> Log out</button></form>@else<a class="flex items-center gap-1.5 px-1 font-semibold text-teal" href="{{ route('login', ['redirect' => url()->current()]) }}"><i class="fa-solid fa-arrow-right-to-bracket text-[11px]" aria-hidden="true"></i> Log in</a><a class="flex items-center gap-1.5 px-1 font-semibold text-coral" href="{{ route('register') }}"><i class="fa-solid fa-user-plus text-[11px]" aria-hidden="true"></i> Sign up</a>@endauth</div>
